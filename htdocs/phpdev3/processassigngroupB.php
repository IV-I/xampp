...
if(!empty($_POST['username']) && !empty($_POST['project_group'])){
    ...
    $query = "INSERT INTO assigngroupb(username, project_group) VALUES('$username', '$project_group')";

    if(!mysqli_query($db, $query)){
        die(mysqli_error($db));
    }else {
        header("Location: GroupB.php?success=Welcome%20you%20are%20in%20group%20B");
    }
} else {
    header("Location: assigngroupB.php?error=Please%20Fill%20All%20Fields");
}
?>