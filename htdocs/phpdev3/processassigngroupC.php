...
if(!empty($_POST['username']) && !empty($_POST['project_group'])){
    ...
    $query = "INSERT INTO assigngroupc(username, project_group) VALUES('$username', '$project_group')";

    if(!mysqli_query($db, $query)){
        die(mysqli_error($db));
    }else {
        header("Location: GroupC.php?success=Welcome%20you%20are%20in%20group%20C");
    }
} else {
