...
if(!empty($_POST['username']) && !empty($_POST['project_group'])){
    ...
    $query = "INSERT INTO assigngroupe(username, project_group) VALUES('$username', '$project_group')";

    if(!mysqli_query($db, $query)){
        die(mysqli_error($db));
    }else {
        header("Location: GroupE.php?success=Welcome,%20you%20are%20in%20group%20E");
    }
} else {
